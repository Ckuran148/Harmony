...
async function fetchData() {
    ...
    try {
        ...
        if (!storeSnap.exists()) {
            document.getElementById('alert-section').innerText = "ERROR: Store not found in Database";
            return;
        }
        ...
        const [generalSnap, marketSnap, districtSnap] = await Promise.all([
            ...
            marketName ? getDoc(doc(db, "markets", marketName)) : Promise.resolve({ exists: () => false }),
            districtName ? getDoc(doc(db, "districts", districtName)) : Promise.resolve({ exists: () => false })
        ]);
        ...
        else if (generalData.daypartSchedule) activeSchedule = generalData.daypartSchedule;
        ...
        if (!isClosed) {
            ...
            if (cachedJoltId) {
                ...
                document.getElementById('jolt-list-current').innerHTML = "<li>No Jolt ID</li>";
            }
        }
...
async function fetchChecklists(locationId) {
    ...
    scrollCurrent.scrollTop = 0; scrollUpcoming.scrollTop = 0;
    ...
    try {
        const res = await fetch(JOLT_ENDPOINT, { method: "POST", headers: JOLT_HEADERS, body: JSON.stringify({ query: CHECKLIST_QUERY, variables }) });
        const payload = await res.json();
        const lists = payload.data?.listInstances || [];
        const incomplete = lists.filter(l => !l.completionTimestamp);

        listCurrent.innerHTML = ''; listUpcoming.innerHTML = '';

        if (incomplete.length === 0) {
            listCurrent.innerHTML = '<li style="color: #028a0f;">✅ All Caught Up!</li>';
            return;
        }

        incomplete.sort((a, b) => (a.displayTimestamp || 0) - (b.displayTimestamp || 0));
        ...
        incomplete.forEach(l => {
            ...
            if (disp > nowSeconds) {
                // UPCOMING
                if (disp <= threeHoursLater) upcomingItems.push({ item: l, status: 'upcoming' });
            } else {
                // CURRENT / LATE
                if (dead > 0 && dead < expiredCutoff) return; // Expired > 24h ago
                
                if (dead > 0 && dead < nowSeconds) currentItems.push({ item: l, status: 'late' });
                else currentItems.push({ item: l, status: 'active' });
            }
        });

        if (currentItems.length > 0) currentItems.forEach(obj => renderJoltItem(obj, listCurrent));
        else listCurrent.innerHTML = '<li style="color: #028a0f;">✅ Complete</li>';

        if (upcomingItems.length > 0) upcomingItems.forEach(obj => renderJoltItem(obj, listUpcoming));
...
function renderJoltItem(dataObj, container) {
    ...
    let rawName = (item.instanceTitle && item.instanceTitle.trim()) || (item.listTemplate && item.listTemplate.title) || `Checklist ${item.id}`;
    const li = document.createElement('li');
    li.className = "jolt-item";
    if (rawName.includes('🟧')) li.classList.add('jolt-important');
    const cleanName = rawName.replace(/([\u2700-\u27BF]|[\uE000-\uF8FF]|\uD83C[\uDC00-\uDFFF]|\uD83D[\uDC00-\uDFFF]|[\u2011-\u26FF]|\uD83E[\uDD10-\uDDFF])/g, '').trim();

    if (status === 'late') { li.classList.add('jolt-late'); li.innerHTML = `⚠️ ${cleanName}`; } 
    else { li.textContent = cleanName; }
...
async function fetchSensors(locationId) {
    ...
    const variables = { 
        mode: { mode: "CONTENT_GROUP", id: "Q29udGVudEdyb3VwOjAwMDU4NzViMjYwZDRjNmI2NDdhNjBjZDAxNDFlZDU2" }, 
        filter: FILTER_DATA,
        scenarioEventsFilter: SCENARIO_FILTER
    };

    try {
        const res = await fetch(JOLT_ENDPOINT, { method: "POST", headers: JOLT_HEADERS, body: JSON.stringify({ query: SENSOR_QUERY, variables }) });
...
function renderSensorCard(sensor, grid) {
    const displayName = sensor.name.includes("-") ? sensor.name.split("-").slice(1).join("-").trim() : sensor.name.trim();
    const tempReading = sensor.readings.find(r => r.type === "VOLTAGE" || r.type === "TEMPERATURE");
    let tempF = "N/A";
    if (tempReading && !isNaN(tempReading.value)) tempF = ((tempReading.value * 9/5) + 32).toFixed(1) + "°F";

    const offline = sensor.signalStrength === "No Signal";
    ...
    if(offline) {
        ...
        const tempEvents = sensor.scenarioEvents.filter(ev => ev.isActive && ev.readingType === "TEMPERATURE");
        if (tempEvents.some(ev => ev.alertType === "ALERT")) tempClass = "temp-critical";
        else if (tempEvents.some(ev => ev.alertType === "WARNING")) tempClass = "temp-warning";
    }

    let batHtml = '<span class="battery-good">🔋</span>';
    if (sensor.batteryLevel <= 20) batHtml = '<span class="battery-low">🪫</span>';
    else if (sensor.batteryLevel <= 59) batHtml = '<span class="battery-med">🔋</span>';
...
function startJoltScroll(container, listElement, type) {
    ...
    const interval = setInterval(() => {
        ...
        if (container.scrollTop >= listElement.scrollHeight) container.scrollTop = 0;
    }, 50);
...
function rotateAlert() {
    ...
    let bgColor = "#333"; let textColor = "#fff";
    for (let i = 0; i < activeSchedule.length; i++) {
        ...
        if (currentMinutes >= startMin && currentMinutes < endMin) {
            ...
            bgColor = phase.color; textColor = phase.textColor; break; 
        }
    }
    const dpDiv = document.getElementById('daypart-section');
    dpDiv.innerText = text; dpDiv.style.backgroundColor = bgColor; dpDiv.style.color = textColor;
}

function checkClosure(datesList) {
    const overlay = document.getElementById('closed-overlay');
    if (!datesList || datesList.length === 0) { overlay.style.display = 'none'; return false; }
    const now = new Date();
    const year = now.getFullYear(); const month = String(now.getMonth() + 1).padStart(2, '0'); const day = String(now.getDate()).padStart(2, '0');
    const todayStr = `${year}-${month}-${day}`;
    if (datesList.includes(todayStr)) { overlay.style.display = 'flex'; return true; } 
    else { overlay.style.display = 'none'; return false; }
}
    ...
    if (list.children.length === 0) list.innerHTML = "<li>No updates.</li>";
        ...
        function updateLTOList(elementId, items) {
            ...
            if (items && items.length > 0) items.forEach(item => {
                const li = document.createElement("li"); li.className = "lto-item";
        ...
        function formatTimeDisplay(timeStr) {
            ...
            const suffix = hours >= 12 ? 'PM' : 'AM'; const displayHour = hours % 12 || 12; const displayMin = minutes.toString().padStart(2, '0'); 
