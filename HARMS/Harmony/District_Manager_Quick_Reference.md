# Harmony Portal — District Manager Quick Reference
...
Your dropdown contains two types of entries:

| Entry | Scope | Use When |
|-------|-------|----------|
| **District: [YourDistrict]** | All stores in your district | Setting defaults for your district |
...
Company (General)  ← Admin only
  └── Operator     ← Operator only (you cannot edit)
        └── District   ← Your defaults for all stores
              └── Store    ← Override per store
```

Lower levels override higher levels. Content from Company and Operator levels will already appear on your stores' displays. Set district defaults first, then override at store level only where needed.
